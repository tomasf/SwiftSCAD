...
extension Geometry3D {
    ...
    @UnionBuilder public func repeated(along axis: Axis3D, in range: Range<Double>, count: Int) -> Geometry3D {
...
extension Geometry2D {
    ...
    @UnionBuilder public func repeated(along axis: Axis2D, in range: Range<Double>, count: Int) -> Geometry2D {
        let step = (range.upperBound - range.lowerBound) / Double(count)
        for value in stride(from: range.lowerBound, to: range.upperBound, by: step) {
            translated(Vector2D(axis: axis, value: value))
        }
